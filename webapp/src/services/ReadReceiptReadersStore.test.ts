...
describe('ReadReceiptReadersStore', () => {
    ...
    it('batches readers requests made in the same tick', async () => {
        const fetchMock = jest.fn(() => createFetchResponse({
            ...
            posts: {
                postA: {count: 1, readers: [{user_id: 'userA', updated_at: 1}]},
                postB: {count: 2, readers: []},
            },
        }));
        (global as any).fetch = fetchMock;

        const [postA, postB] = await Promise.all([
            fetchReadReceiptReaders('postA'),
            fetchReadReceiptReaders('postB'),
        ]);

        expect(fetchMock).toHaveBeenCalledTimes(1);
        const init = (fetchMock.mock.calls[0] as unknown as [string, RequestInit])[1];
        expect(JSON.parse(String(init.body))).toEqual({post_ids: ['postA', 'postB']});
    ...
    it('serves cached readers without another request', async () => {
        const fetchMock = jest.fn(() => createFetchResponse({
            max_readers_per_post: 50,
            posts: {
                postA: {count: 1, readers: []},
            },
        }));
        (global as any).fetch = fetchMock;

        await fetchReadReceiptReaders('postA');
        await fetchReadReceiptReaders('postA');
